...
<button type="button" class="group relative {{ $class }}">
    ...
    <span
        class="flex justify-center items-center gap-2 uppercase script-font fold-normal relative h-full w-full rounded border-2 border-brown-400 bg-yellow-50 px-3 py-1 md:px-6 md:py-2 text-base text-brown-400 transition duration-100 hover:bg-yellow-800 hover:border-yellow-800 hover:text-yellow-50 dark:text-yellow-200 dark:bg-dark dark:border-yellow-200 dark:group-hover:bg-yellow-200 dark:group-hover:text-yellow-900 {{ $reverseDirection ? 'flex-row-reverse' : '' }}">
        {{ $slot }}
        {{ $text }}
    </span>
</button>
