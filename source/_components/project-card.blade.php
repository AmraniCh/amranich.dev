<div
    class="card noscript-opacity-1 opacity-0 group/container {{ $highlight ? 'px-4 py-8 rounded-md bg-white border-2 border-dashed border-yellow-800 dark:bg-brown-900 dark:border-yellow-200' : '' }}">
    <div class="flex flex-col w-full {{ $highlight ? 'lg:flex-row-reverse' : 'lg:flex-row' }} lg:gap-6">
        ...
        <div class="lg:flex lg:items-center lg:flex-shrink-0">
            <img class="w-full rounded-md transition-transform group-hover/container:scale-[1.01] lg:w-[500px] xl:w-[600px]"
                src="/assets/images/{{ $snapshot }}" alt="{{ $snapshotAlt }}">
            ...
            <h5 class="text-sm mt-2 capitalize flex items-center gap-2">
                ...
                @if ($highlight)
                    <div class="flex items-center gap-1">
                        <x-svg.star classes="!static !animate-none !opacity-100" />
                        <x-svg.star classes="!static !animate-none !opacity-100" />
                        <x-svg.star classes="!static !animate-none !opacity-100" />
                    </div>
                @endif
